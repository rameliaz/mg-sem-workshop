...
* ***Root Mean Square Error of Approximation* (RMSEA)** {{< fa circle-arrow-right >}} *close fit* ketika nilainya **< 0.05**, *acceptable fit* ketika **0.05 – 0.08**, *poor fit* ketika **> 0.10**
...
* ***Standardized Root Mean Square Residual* (SRMR)** {{< fa circle-arrow-right >}} rata-rata perbedaan antara *observed* dan *model-implied correlation matrix*, nilai **< 0.08** menunjukkan *acceptable fit*
  - Penting diperhatikan karena RMSEA sensitif terhadap jumlah sampel yang besar (cenderung menolak model yang sebenarnya baik), sedangkan SRMR **tidak bergantung pada jumlah sampel** dan langsung mencerminkan seberapa jauh *implied model* mereproduksi data observasi
...
* *Statistical power* dalam pengujian hipotesis dalam SEM {{< fa circle-arrow-right >}} peluang **menolak *H*~0~** apabila *H*~0~ **salah** (power = 1 − β)
  ...
  - _Power_ dalam konteks SEM yang lebih spesifik adalah peluang peneliti secara tepat menyimpulkan bahwa suatu koefisien (misalnya koefisien jalur struktural) berbeda dari nol, ketika memang benar-benar berbeda dari nol di populasi.
